...
;// jQuery and Bootstrap scripts get added via grunt task!

// Globals
var BLOCK_CLASS = "email-block";

// Templatizer Module
var Templatizer = (function(){
  var templatizer = {};
  var wrapper;

  function checkForBlocks() {
    console.log("Templatizer::checkForBlocks");
    var blocks = wrapper.children('.'+BLOCK_CLASS);
    // Show or hide actions if email blocks exist
    if (blocks.length > 0) {
      $('#template-actions').addClass('show');
    } else {
      $('#template-actions').removeClass('show');
    }

    blocks.each(function() {
      block = $(this);
      if (block.is(':first-child')) {
        block.find('.move-up').addClass('disabled');
      } else {
        block.find('.move-up').removeClass('disabled');
      }

      if (block.is(':last-child')) {
        block.find('.move-down').addClass('disabled');
      } else {
        block.find('.move-down').removeClass('disabled');
      }
    });
  }
  ...
  function addBlockEvents(block) {
    // block input events
    block.on("change keyup", '#block-headline', function(){
      block.find('.block-title').text($(this).val());
    });

    // block button events
    block.on("click", '.block-bg-color a', function(){
      $(this).addClass('active');
      $(this).siblings().removeClass('active');
    });

    block.on("click", ".delete", function(){
      templatizer.removeBlock(block);
    });

    block.on("click", ".move-up", function(){
      templatizer.moveBlockUp(block);
    });

    block.on("click", ".move-down", function(){
      templatizer.moveBlockDown(block);
    });
  }
  ...
  function generateTemplateString() {
    ...
    $.templates({
      topTemplate: getMarkupString('template_top.html'),
      blockTemplate: getMarkupString('template_block.html'),
      bottomTemplate: getMarkupString('template_bottom.html')
    });

    html = $.render.topTemplate();

    var blocks = wrapper.children('.'+BLOCK_CLASS);
    blocks.each(function() {
      var block = $(this);
      var styleBtn = $(block.find('.block-bg-color .active').get(0));
      var blockData = {
        color: {
          bg: styleBtn.data('bg'),
          title: styleBtn.data('title'),
          text: styleBtn.data('text')
        },
        headline: block.find('#block-headline').val(),
        content: block.find('#block-content').val(),
        btn: {
          text: block.find('#block-btn-text').val(),
          link: block.find('#block-btn-link').val()
        },
        img: {
          path: block.find('#block-img-path').val(),
          width: block.find('#block-img-width').val(),
          height: block.find('#block-img-height').val(),
          alt: block.find('#block-img-alt').val(),
          link: block.find('#block-img-url').val()
        },
        ga: {
          source: block.find('#block-ga-source').val(),
          campaign: block.find('#block-ga-campaign').val()
        }
      };

      html = html + $.render.blockTemplate( blockData );
    });

    html = html + $.render.bottomTemplate();

    return html;
  }

  function getMarkupString(htmlFile) {
    var response;
    $.ajax({ type: "GET",
      url: htmlFile,
      async: false,
  ...
  templatizer.addBlock = function() {
    console.log("Templatizer::addBlock");
    var blockWrapper = $('<div />', {
      "class": BLOCK_CLASS+' well row'
    });
    var newBlock = blockWrapper.load("email_block.html");
    wrapper.append(newBlock);

    addBlockEvents(newBlock);
    setTimeout(checkForBlocks, 100);
  };

  templatizer.removeBlock = function(block) {
    block.remove();
    checkForBlocks();
  };

  templatizer.moveBlockUp = function(block) {
    var prevBlock = block.prev('.'+BLOCK_CLASS);
  ...
  templatizer.moveBlockDown = function(block) {
    var nextBlock = block.next('.'+BLOCK_CLASS);
  ...
  templatizer.getTemplateString = function() {
    return generateTemplateString();
  };

  templatizer.init = function(wrapperElement) {
     wrapper = $(wrapperElement);
     console.log("Templatizer initialized with: ", wrapper.attr('id'));
  };
...
$(document).ready(function(){
  var wrapperId = "#email-blocks";

  $('#insert-btn').on("click", function(){
    Templatizer.addBlock();
  });

  $('#preview').on("click", function(e){
    var iFrame = $("<iframe />");
    var templateString = Templatizer.getTemplateString();
    iFrame.attr('src', 'data:text/html;charset=utf-8,' + encodeURIComponent(templateString));
  ...
  $('.download').on("click", function(e){
    ...
    var templateString = Templatizer.getTemplateString();
    download.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(templateString));
    download.setAttribute('download', "EmailTemplate.html");
    download.click();
  });

  Templatizer.init($(wrapperId));
});
